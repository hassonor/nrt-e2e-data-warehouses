...
class PinotTableSubmitOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> Optional[str]:
        ...
        if not table_files:
            self.log.info(
                "No table files found in %s. Nothing to submit.", self.folder_path
            )
            return "No tables submitted."

        self.log.info(
            "Found %d table file(s) in %s.", len(table_files), self.folder_path
        )
        ...
        for table_file in table_files:
            ...
            try:
                ...
                response = requests.post(
                    self.pinot_url, data=table_data, headers=headers
                )
                if response.status_code == 200:
                    self.log.info("Table successfully submitted: %s", table_file)
                else:
                    self.log.error(
                        "Failed to submit table file %s: %d - %s",
                        table_file,
                ...
                self.log.exception(
                    "An unexpected error occurred while submitting %s", table_file
                )
